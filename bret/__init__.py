...
def set_payoff(player: Player):
    ...
    if round_number == 1:
        participant.vars['round_to_pay'] = random.randint(1, NUM_ROUNDS) # todo possible bug: this means that the pay_off round could be the round where the user scored 0(bomb was selected). is that expected?
    if RANDOM_PAYOFF:
...
class Game(Page):
    ...
    def js_vars(player: Player):
        ...
        if DYNAMIC:
            ...
            input = not DEVILS_GAME #comment in git - failed to fix -- this logic had been reversed. For DEVILS_GAME==True, an input field was shown to the user to enter the number of boxes to choose. This corrects that bug
